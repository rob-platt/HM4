# This is a script to collate all of the mineral pixels detailed in the
# Machine Learning Toolkit for CRISM Analysis from the original images
# https://github.com/Banus/crism_ml/tree/master.
# This requires the CRISM_labeled_pixels_ratioed.mat file from
# https://zenodo.org/records/13338091. This must be in the data/CRISM_ML dir.
# This also requires the images to be formatted in the following file
# structure:
...
# The data is saved to the data/extracted_mineral_pixel_data dir as a JSON
# file.
...
try:
    ...
    raise IOError(
        f"Could not load the CRISM_labeled_pixels_ratioed.mat"
        f"from {CRISM_ML_DIR}"
    )
...
# Get the image name for each pixel in the .mat dataset.
# Initially are numerically encoded, so also decode.
# Image names given as the unique 5 long hexadecimal shortcodes (e.g. 3561F)
...
# Original file is matlab, which is 1-indexed, python is 0-indexed so
# -1 to index position to account for that
...
for idx, image_name in enumerate(downloaded_images_list):
    ...
    pixel_class_list = []
    start_time = time()
    # Check if the current image (slicing for the shortcode)
    # contains bland pixels in the .mat file
    ...
    # Mask of the bland pixels in this image, from the list of all the
    # bland pixels across all the images
    ...
    # Loop through coords of bland pixels in that image, slice the image array
    # to get the spectra for that pixel, and add all the information to the
    # lists.
    for coords, label in zip(relevant_pixel_coords, relevant_pixel_labels):
        x, y = coords
        # Some of the pixel coordinates are [0,0] which is invalid
        # a) Matlab is 1 indexed, b) often there are multiple pixels
        # attributed to the same [0,0] which isn't possible.
        if x > image_shape[1] or y > image_shape[0] or x == 0 or y == 0:
            continue
        spectra_list.append(
            image_array[y - 1, x - 1]
        )
        # image_array is (n_rows, n_cols, n_bands) so flipped x and y.
        # -1 offset to account for python indexing starting at 0 but
        # coordinates starting at (1, 1).
        coords_list.append(coords)
        image_name_list.append(image_name[-5:])  # Hexadecimal shortcode only
        pixel_class_list.append(label)

    # Create a dataframe of that images pixels and save it to a
    # JSON file in a temporary dir
    ...
    projected_time = (
        ((total_time_taken / (idx + 1)) *
         (len(downloaded_images_list) - (1 + idx))) / 60
    ) + 5
    print(
        f"Finished {image_name} in {finish_time - start_time:.2f} seconds, "
        f"projected completion time: {projected_time:.2f} minutes",
        flush=True,
    )
...
print(
    f"Finished collating all the images into a single dataset,"
    f"saved to {join(OUTPUT_DIR, 'mineral_pixel_dataset.json')}",
    flush=True,
)
